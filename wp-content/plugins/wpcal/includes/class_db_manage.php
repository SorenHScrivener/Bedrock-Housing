...
class WPCal_DB_Manage {
	...
	private static $wpdb;
	private static $db_version = '0.9.5.2'; //Change DB version only when db changes are there - this will make sure rechecking each time(php instance) through all versions update functions
	...
	private static function create_table_calendars() {
		...
		$query = "CREATE TABLE IF NOT EXISTS `$table_name` (
			...
			KEY `events_webhook_resource_id` (`events_webhook_resource_id`)
		  ) ENGINE=InnoDB " . self::$collation;
	...
	private static function may_add_current_wp_admin_as_wpcal_admin() {
		...
		$result = $wpdb->insert($table_admins, $insert_data);
	}
	...
	public static function update() {
		...
		self::modify_validity_check_time_to_trigger_soon();
	...
	public static function may_add_after_update_whats_new_notice($wpcal_version) {
		...
		if (version_compare($wpcal_version, '0.9.5.0', '<') && WPCAL_VERSION == '0.9.5.2') {
		...
		<a class="btn" href="https://wpcal.io/whats-new-in-v0-9-5/?utm_source=wpcal_plugin&utm_medium=after_update_notice" target="_blank">Open the blog post ↗</a>';
			...
			WPCal_Manage_Notices::add_notice($notice_data);
		}
	}

	private static function modify_validity_check_time_to_trigger_soon() {
		WPCal_License::reset_license_last_timings();
	}
