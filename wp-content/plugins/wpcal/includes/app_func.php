...
function wpcal_mark_onboarding_checklist_completed_for_current_admin($task) {
	...
	if (!in_array($task, ['add_calendar_account', 'add_meeting_app', 'add_wpcal_admin'])) {
		//invalid task
		return;
	}
...
function wpcal_can_reuse_same_meeting_tp_resource_id_on_reschedule(WPCal_Booking $old_booking_obj, WPCal_Booking $new_booking_obj, $add_bookings_to_calendar = null) {
	...
	$tp_account = wpcal_get_active_tp_account_by_admin_and_provider($new_booking_obj->get_admin_user_id(), $provider);
...
function wpcal_get_calendar_details_by_id($calendar_id) {
	...
	$query = "SELECT `cals`.`id` as `calendar_id`, `cals`.`calendar_account_id`, `cal_accs`.`provider`, `cals`.`tp_cal_id` FROM `$table_calendar_accounts` as `cal_accs` JOIN `$table_calendars` as `cals` ON `cal_accs`.`id` = `cals`.`calendar_account_id` WHERE `cal_accs`.`status` = '1' AND `cals`.`status` = '1' AND `cals`.`id` = %s";
...
function wpcal_may_update_or_delete_booking_to_tp_calendar(WPCal_Booking $booking_obj, $do) {
	...
	$calendar_id = wpcal_get_active_calendar_id_from_booking_event_added_may_use_alternate_methods_to_find($booking_obj);
...
function wpcal_get_active_calendar_id_from_booking_event_added_may_use_alternate_methods_to_find(WPCal_Booking $booking_obj) {
	// this is only for calendar update or delete event
	...
	$add_booking_to_cal_details = wpcal_get_add_events_calendar_by_admin($admin_user_id);
	...
	// check the same calendar_id exists for the same admin
	$query = "SELECT `calendars`.`id` FROM `$table_calendars` as `calendars` JOIN `$table_calendar_accounts` as `calendar_accounts` ON `calendar_accounts`.`id` = `calendars`.`calendar_account_id` WHERE `calendar_accounts`.`status` = '1' AND `calendars`.`id` = %s AND `calendars`.`status` = '1' AND `calendar_accounts`.`admin_user_id` = %s";
	...
	$query2 = "SELECT `calendars`.`id` FROM `$table_calendars` as `calendars` JOIN `$table_calendar_accounts` as `calendar_accounts` ON `calendar_accounts`.`id` = `calendars`.`calendar_account_id` WHERE `calendar_accounts`.`status` = '1' AND `calendars`.`tp_cal_id` = %s AND `calendars`.`status` = '1' AND `calendar_accounts`.`admin_user_id` = %s";
...
function wpcal_google_calendar_receive_token_and_add_account() {
	...
	$tp_calendar_obj = new $tp_calendar_class(0);
	$add_account_result = $tp_calendar_obj->add_account_after_auth();
