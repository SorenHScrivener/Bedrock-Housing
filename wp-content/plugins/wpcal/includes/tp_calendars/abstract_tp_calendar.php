...
abstract class WPCal_Abstract_TP_Calendar {
	...
	// protected function is_status_ok() {
	// 	if (empty($this->cal_account_details)) {
	// 		throw new WPCal_Exception('calendar_account_details_not_loaded');
	// 	}
	// 	$status = $this->cal_account_details->status;

	// 	if (!is_numeric($status) || $status < -10 || $status > 10 || $status != (int) $status) {
	// 		throw new WPCal_Exception('calendar_account_status_invalid');
	// 	}
