...
class WPCal_License {
	...
	private static function save_creds_info($creds) {
		...
		$whitelist = array('email', 'site_token', 'last_validated', 'last_checked', 'status', 'expiry', 'validity_error', 'issue_deducted', 'plan_slug', 'plan_name', 'additional_details');
	...
	public static function is_valid($cache = false) { //check the db
		...
		if ($creds['status'] === 'valid' && $creds['last_validated'] > time() - (13 * 60 * 60)) {
			return true;
		} elseif ($cache === false && $creds['last_checked'] < time() - (13 * 60 * 60)) {
			self::check_validity();
	...
	public static function reset_license_last_timings() {
		$creds = self::get_creds_info();

		if (isset($creds['last_validated']) || isset($creds['last_checked'])) {
			$time_now = time();
			$reset_to = $time_now - (86400 * 2);

			if (isset($creds['last_validated']) && $creds['last_validated']) {
				$creds['last_validated'] = $reset_to;
			}
			if (isset($creds['last_checked']) && $creds['last_checked']) {
				$creds['last_checked'] = $reset_to;
			}
			self::save_creds_info($creds);
		}
	}
